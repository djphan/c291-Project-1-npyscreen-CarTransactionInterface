...
class ViolationRecord(npyscreen.ActionForm):
    def create(self):
        ...
        self.sin = self.add(npyscreen.TitleText, name='Violator SIN:',
                            begin_entry_at=20)
        self.vehicle_no = self.add(npyscreen.TitleText,
                                   name='Vehicle Serial No:',
                                   use_two_lines=False, begin_entry_at=20)
        self.button10 = self.add(npyscreen.ButtonPress,
                                 name="Use Vehicle's Primary Owner SIN")
        ...
        self.place = self.add(npyscreen.TitleText, name='Place:',
                              begin_entry_at=20)
        self.nextrely+=1
        self.d_title    = self.add(npyscreen.TitleFixedText,
                                   use_two_lines=False, name="Description:",
                                   begin_entry_at=20, editable=False,
                                   color="STANDOUT")
        self.description = self.add(npyscreen.MultiLineEdit,
                                    name='Description:', relx=22, rely=12)

        # Get maximum current ticket_id
        query = "SELECT MAX(ticket_no) FROM ticket"
        # Set t_id to one greater

        # Try to autoincrement the highest t_id in the db; if no ticket exists
        # in the db set the initial t_id to 100.
        try:
    ...
    def process_information(self):
        self.entries = {'t_id' : self.t_id.value,
                        ...
                        "t_date" :self.date.value.strftime("%d-%b-%y"),
    ...
    def button_press_primaryid(self):
        ...
        try:
            self.sin.value = self.parentApp.db.query(
                {'vehicle_no':self.vehicle_no.value.ljust(15, ' ')},query)[0][0]
        # Otherwise, input is wrong and notify user
        except:
            npyscreen.notify_confirm(
                "Invalid Vehicle No entered. Cannot find primary owner.",
    ...
    def validate_forms(self):
        ...
        if self.parentApp.db.query(
            ...
            npyscreen.notify_confirm("Invalid Violator SIN entered",
        ...
        if self.parentApp.db.query(
            {'ser':self.vehicle_no.value.ljust(15, ' ')}, query)[0][0] == 0:
            npyscreen.notify_confirm(
                "Invalid vehicle serial number. "
                "The vehicle is not registered in the database.", title="Error",
        ...
        query = "SELECT COUNT(sin) FROM people WHERE sin = :sin"
        if self.parentApp.db.query(
            {'sin':self.officer_no.value.ljust(15, ' ')}, query)[0][0] == 0:
            npyscreen.notify_confirm(
                "Invalid Officer ID. "
                "Officer not in people database", title="Error",
                form_color='STANDOUT', wrap=True, wide=False, editw=1)
            return False
        ...
        if self.parentApp.db.query(
            {'v_type':self.violation_type.value.ljust(10, ' ')}, query)[0][0] == 0:
            npyscreen.notify_confirm(
                "Invalid Violation Type."
                "Type ID not in violation type database", title="Error",
                form_color='STANDOUT', wrap=True, wide=False, editw=1)
            return False
